...
var MODE_RESET = 3;       //single click to reset the map to full extent
var MODE_PAN = 4;         //click and drag to pan the image
var MODE_SET_AOI = 5;     //click and drag to set a bbox without any change to the image
...
function mouseUpHandler(ev) {
  ...
  switch(this.mode) {
    ...
    case 'MODE_SET_AOI':				//setting AOI
      ...
      break;
    case 'MODE_GETFEATUREINFO':
      this.featureInfo.get(this.evpl);
      break;	
    default:
      //alert("invalid mode:" +this.mode);
      break;
    case 'MODE_ALT_CLICK':		  //zoom by ALT/CTRL click
      if (this.altKey) {
        this.context.extent.CenterAt(this.evxy, this.context.extent.res[0]/zoomby);
