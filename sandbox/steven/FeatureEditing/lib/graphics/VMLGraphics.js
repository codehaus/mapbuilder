...
VMLGraphics.prototype.setShapeSize = function(shape, x) {;
	var oldSize = parseInt(shape.style.width.replace("px",""));
	if(oldSize=="NaN"||!oldSize) return false;
  shape.style.width= x*2;
  shape.style.height= x*2;
	var oldLeft = parseInt(shape.style.left.replace("px",""));
		if(oldLeft=="NaN"||!oldLeft) return false;
	var oldTop = parseInt(shape.style.top.replace("px",""));
...
VMLGraphics.prototype.drawCircle=function( X, Y, radius) {
  ...
  element.style.width=diameter;//diameter;
  element.style.height=diameter;//diameter;
