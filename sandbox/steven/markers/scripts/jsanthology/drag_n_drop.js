...
//alert(document.getElementById("dragLegende").getElementsByTagName("div"));
  var LIs= document.getElementById("sortableLayerList").getElementsByTagName("li");;
...
function mousedownDragNDrop(event)
{
  ...
  while (target.nodeName.toLowerCase() != "div")
  {
	...
	alert(Sarissa.serialize(target));
  document.currentTarget = target;
alert(Sarissa.serialize(target.parentNode));
...
function mouseupDragNDrop(event)
{
  ...
  var hotZone = document.getElementById("sortableLayerControl");
  ...
  else
  {
    ...
    config.objects.editContext.addNodeToModel(target.parentNode.id)
