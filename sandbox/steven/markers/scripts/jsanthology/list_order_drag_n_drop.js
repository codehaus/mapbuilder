...
function deleteSortableList(widgetId,targetId)
{
  id=widgetId;
  model=targetId;
  ...
  for (var i = 0; i < LIs.length; i++)
  {
    detachEventListener(LIs[i], "mousedown", mousedownSortableList, false);
    LIs[i].style.cursor = "";
  }
}
  ...
  for (var i = 0; i < LIs.length; i++)
  {
    if (LIs[i] != target && LIs[i] != target.clone)
    {
      if (event.pageY < getPosition(LIs[i])[1] + LIs[i].offsetHeight && currentItemHigher)
      {
        ...
        if(id=="layerControl")config.objects.mainMap.setParam('moveLayerUp',LIs[i].getAttribute("id"));
        break;
      }
      else if (event.pageY > getPosition(LIs[i])[1] && !currentItemHigher)
      {
        target.parentNode.insertBefore(LIs[i], target);
        if(id=="layerControl")config.objects.mainMap.setParam('moveLayerDown',LIs[i].getAttribute("id"));
      }
    }
    else
    {
