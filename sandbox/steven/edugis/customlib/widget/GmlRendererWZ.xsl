...
$Id: GmlRendererWZ.xsl,v 1.13 2005/09/21 11:34:45 rabla Exp $
$Name:  $
-->
...
<xsl:variable name="x0" select="floor((number(gml:X)-$bBoxMinX)*$xRatio - number($pointWidth) div 2)"/>
<xsl:variable name="y0" select="floor($height - (number(gml:Y)-$bBoxMinY)*$yRatio - $pointWidth div 2)"/>

          // Point
          <xsl:value-of select="$objRef"/>.jg.fillRect(
