...
function mkLinDott_Img(x1, y1, x2, y2)
{
    ...
    var imageWidth = 6;
    var space = 1; // --> n * imageWidth
    
    var dx = Math.abs(x2-x1), dy = Math.abs(y2-y1);
    var x = x1-(imageWidth/2), y = y1-(imageWidth/2);
    var signY = (y1 > y2)? -1:1;
    var signX = (x1 > x2)? -1:1;
    
    var c = Math.sqrt((dx*dx)+(dy*dy));
    var dots = (c / (imageWidth+(imageWidth*space)))-1;
    var cIncr = (c/dots);
    ...
    if (dx == 0 && dy == 0){
        ...
        if (dx != 0 && dy != 0){
            ...
            var yIncr = signY * (cIncr*Math.sin(alpha));                  
            var xIncr = signX * (cIncr*Math.cos(alpha)); 
        }
        if (dx == 0 && dy != 0){
            var yIncr = signY * (dy/cIncr);                  
            var xIncr = 0;
        }
        if (dy == 0 && dx != 0){
            var yIncr = 0;                  
            var xIncr = signX * (dx / cIncr);
        }
            
        this.drawImage(image, Math.round(x), Math.round(y), imageWidth, imageWidth);
        for (i=1; i<dots; i++)
        {
            x += xIncr;
            y += yIncr;
        ...
        this.drawImage(image, x2-(imageWidth/2), y2-(imageWidth/2), imageWidth, imageWidth);
    }
}
