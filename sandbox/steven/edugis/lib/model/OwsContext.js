...
$Id: OwsContext.js 2121 2006-06-12 21:35:13Z lderezinski $
*/
...
function OwsContext(modelNode, parent) {
 ...
 /**
   ...
   * @param layerName  The name of the layer that is to be changed
   * @param transparancy     String with the value to be set; 
   */
  this.setTransparancy=function(layerName, transparancy){
    // Set the transparancy attribute in the Context

      
    var layer=this.getFeatureNode(layerName);
    layer.setAttribute("transparancy", transparancy);
    // Call the listeners
    this.callListeners("transparancy", layerName);
  }

  /**
   * Get the layer's transparancy attribute value.
   * @param layerName  The name of the layer that is to be changed
   * @return transparancy  String with the value; 
   */
  this.getTransparancy=function(layerName){
    var transparancy="100";
    var layer=this.getFeatureNode(layerName)
    return layer.getAttribute("transparancy");
  }
  /**
   * Get the BoundingBox.
  ...
  /**
   * Method to get the zoomLevels with the specified name in the context doc
   * @param layerName name of the layer from which the zoomLevels should be retrieved
   * @return the list of zoomLevels
   */
  this.getZoomLevels = function(layerName) {
    var zoomLevelNode = this.doc.selectSingleNode("/wmc:OWSContext/wmc:ResourceList/wmc:Layer[wmc:Name='"+layerName+"']/wms-c:ZoomLevels");
    if (zoomLevelNode) {
      var zoomLevels = zoomLevelNode.firstChild.nodeValue.split(",");
      return zoomLevels;
    }
    return null;
  }
