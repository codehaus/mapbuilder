...
OpenLayers.Util.getSegmentSnappingPoint = function(position, geometry) {
    var point = null;
    var pointIndex = null;
    var segment = null;
    var segmentDelta = null;
    var snappingGeometry = null;
    
    if(geometry) {    
        if (geometry.path) {
            
            snappingGeometry = geometry;
            ...
            for(var i = 0; i < geometry.path.length - 1; i++) {
                ...
                // Calculate the segment function parameters               
                var m1 = (point2.y - point1.y) / (point2.x - point1.x);
                var p1 = point1.y - m1 * point1.x;
                var tmpSegment = new  OpenLayers.Geometry.LineSegment(point1, point2);
                ...
                if (m1 == 0) {
                    ...
                    var m2 = - (1 / m1);
                    var p2 = position.y - m2 * position.x;
                    var x = (p2 - p1) / (m1 + 1 / m1);
                    var y = (m1 * x + p1);
                }
                ...
                var tmpPoint = new OpenLayers.Geometry.Point(x, y);
                
                var tmpDelta = Math.sqrt(Math.pow((tmpPoint.x - position.x), 2) + Math.pow((tmpPoint.y - position.y), 2));
    
                // verify the limit of the function and if the distance is shorter as the others
                if(!segmentDelta || (tmpDelta < segmentDelta && (Math.min(point1.x, point2.x) <= position.x && position.x <= Math.max(point1.x, point2.x)))){
                    point = tmpPoint;
                    segment = tmpSegment;
                    segmentDelta = tmpDelta;
                    pointIndex = i;
                }
            }
            return {point: point,
            ...
            for(var i = 0; i < geometry.components.length; i++) {
                var tmpResult = OpenLayers.Util.getSegmentSnappingPoint(position, geometry.components[i]);
                if (!result || (tmpResult.distance < result.distance)) {
                    result = tmpResult;
                }
            }
            return result;
...
OpenLayers.Util.getVertexSnappingPoint = function(position, geometry) {
    var point = null;
    var index = null;
    var delta = null;
    var snappingGeometry = null;
    if (geometry) {  
        if (geometry.path) {
            ...
            for(var i = 0; i < snappingGeometry.path.length; i++) {
                // Set segment points
                var vertex = snappingGeometry.path[i];
    			var tmpDelta = Math.sqrt(Math.pow((position.x - vertex.x), 2) + Math.pow((position.y - vertex.y), 2));
                
                if(!delta || delta > tmpDelta){
                    point = vertex;
                    delta = tmpDelta;
                    index = i;
                }
            ...
            for(var i = 0; i < geometry.components.length; i++) {
                var tmpResult = OpenLayers.Util.getVertexSnappingPoint(position, geometry.components[i]);
                if (!result || (tmpResult.pathPointDelta < result.pathPointDelta)) {
                    result = tmpResult;
                }
            }
            return result;
