...
OpenLayers.Map.prototype = {
    ...
    EVENT_TYPES: [
        ...
        "changebaselayer", "changevectorlayer", "Editing", "insertfeature"],
  ...
  // Options
    ...
    /** @type OpenLayers.Control.EditingState */
    editingState: null,
    ...
    initialize: function (div, options) {
		...
		this.loadingDiv = OpenLayers.Util.createDiv(id, null, null, null,
                                                     "relative", null,
                                                     "hidden");
    ...
    //
        ...
        this.editingModes = [];
        this.snappingTolerance = 10;
        //this.addEditingMode(new OpenLayers.Control.EditingMode.PointSnapping());
        //this.addEditingMode(new OpenLayers.Control.EditingMode.SegmentSnapping());
    },
  ...
  /********************************************************/
    ...
    */
    addLayer: function (layer) {
        ...
        if (layer.isFixed) {
            if (layer.isVector) {
                layer.setZIndex(this.Z_INDEX_BASE['Popup']+this.layers.length*5);
            }
    ...
    setVectorLayer: function(newVectorLayer, noEvent) {
        ...
        if (newVectorLayer != oldVectorLayer) {
            ...
            if (OpenLayers.Util.indexOf(this.layers, newVectorLayer) != -1) {
                ...
                this.setLayerIndex(newVectorLayer, this.layers.length);

                //  trigger a changevectorlayer event
                if ((noEvent == null) || (noEvent == false)) {
    ...
    */
    ...
    addControlToMap: function (control, px) {
        ...
        if(!div.parentNode || (div.parentNode.nodeName == "#document-fragment")) {
    ...
    /**
     * @param {OpenLayers.MouseListener} mouseListener
     */
    addMouseListener: function(mouseListener) {
        ...
        this.events.register( "mouseover", mouseListener, mouseListener.mouseOver );
        this.events.register( "mouseout", mouseListener, mouseListener.mouseOut );

        // should this be here ?
        this.events.register( "keydown", mouseListener, mouseListener.keyDown );

        mouseListener.mouseWheel = mouseListener.mouseWheel.bindAsEventListener(mouseListener);
        OpenLayers.Event.observe(window, "DOMMouseScroll",  mouseListener.mouseWheel, false);
        OpenLayers.Event.observe(window, "mousewheel",  mouseListener.mouseWheel, false);
        OpenLayers.Event.observe(document, "mousewheel",  mouseListener.mouseWheel, false);


        // Observe the Keyboard up and down events
        // TBD should this be here ? (mouseListener ?)
        if (mouseListener.keyDown) {
            mouseListener.keyDown = mouseListener.keyDown.bindAsEventListener(mouseListener);
            OpenLayers.Event.observe(document, "keydown", mouseListener.keyDown);
        }
        if (mouseListener.keyUp) {
            mouseListener.keyUp = mouseListener.keyUp.bindAsEventListener(mouseListener);
            OpenLayers.Event.observe(document, "keyup", mouseListener.keyUp);
        }
    ...
    removeMouseListener: function(mouseListener){
        ...
        this.events.unregister( "mouseover", mouseListener, mouseListener.mouseOver );
        ...
        if (mouseListener.keyDown) {
        	OpenLayers.Event.stopObserving(document, "keydown", mouseListener.keyDown);
        }

        if (mouseListener.keyUp) {
        	OpenLayers.Event.stopObserving(document, "keyup", mouseListener.keyUp);
		}
    ...
    addEditingMode: function(editingMode) {
        editingMode.setMap(this);
        this.editingModes.push(editingMode);
    },

    removeEditingMode: function(editingMode) {
        this.editingModes = OpenLayers.Util.removeItem(this.editingModes, editingMode);
    },
  ...
  /********************************************************/
    ...
    /**
     * @returns The resolution for the provided zoom level.
     *          If no zoom is passed, returns the current resolution of the map.
     *          If no baselayer is set, returns null.
     * @type float
     */
    getResolution: function (zoom) {
        var resolution = null;
        if (this.baseLayer != null) {
            resolution = this.baseLayer.getResolution(zoom);
        }
        return resolution;
    },
  ...
  //
    ...
    */
    getLonLatFromLayerPx: function (px) {
       ...
       var px = this.getViewPortPxFromLayerPx(px);
       return this.getLonLatFromViewPortPx(px);
    },
