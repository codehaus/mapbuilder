...
<xsl:stylesheet xmlns:wfs="http://www.opengis.net/wfs"
			...
			<xsl:choose>
				...
				<xsl:otherwise>
					<mb:QueryString>
						...
						<xsl:value-of
							select="translate(normalize-space($query),' ','')"
	...
	<xsl:template
		match="wmc:FeatureType[wmc:Server/@service='OGC:WFS']">
		<xsl:param name="resourceName" select="wmc:Name" />
		<xsl:param name="featureSrs" select="wmc:SRS" />
		<GetFeature xmlns="http://www.opengis.net/wfs" version="1.0.0"
			service="WFS" maxFeatures="{$maxFeatures}">
			<xsl:choose>
				<xsl:when test="$httpMethod='post'">
					<Query typeName="{$resourceName}">
						<ogc:Filter>
							<ogc:And>
								<xsl:if test="wmc:Geometry">
									<xsl:variable name="bbox"
										select="concat($bBoxMinX,concat(',',concat($bBoxMinY,concat(' ',concat($bBoxMaxX,concat(',',$bBoxMaxY))))))" />
									<ogc:BBOX>
										<ogc:PropertyName>
											<xsl:value-of
												select="wmc:Geometry" />
				...
				<xsl:otherwise>
					<mb:QueryString>
						...
						<xsl:value-of
							select="translate(normalize-space($query),' ','')"
							disable-output-escaping="no" />
						<xsl:if test="$cFilterStr">
							&amp;filter=
							<xsl:value-of
								select="normalize-space($cFilterStr)" />
						</xsl:if>
					</mb:QueryString>
				</xsl:otherwise>
			</xsl:choose>
