...
function WfsGetFeature(widgetNode,model){
	...
	this.doAction=function(objRef,targetNode){
		if(objRef.enabled){
			...
			objRef.transactionResponseModel.newRequest(transaction,objRef.httpPayload);
			objRef.loadDefaultModel(objRef);
			
			//TODO: THIS DOESN't WORK! Why NOT?
			objRef.transactionResponseModel.setXpathValue(objRef.transactionResponseModel,objRef.featureXpath,transaction.doc);
		}
	}
	this.loadDefaultModel=function(objRef)
	{
		objRef.transactionResponseModel.url=objRef.defaultModelUrl;
		var httpPayload=new Object();
		httpPayload.url=objRef.defaultModelUrl;
		httpPayload.method="get";
		httpPayload.postData=null;
		objRef.transactionResponseModel.newRequest(objRef.transactionResponseModel,httpPayload);
	}
