...
//added NR
  ...
  while ($myrow = mysql_fetch_array($result)){
      ...
      $depth++;
      $atts['fid']=$myrow["FeatureID"];
      writeTag("open",null,$table,$atts,True,True);
      $depth++;   
...
function readRequest(){
  ...
  switch($REQUEST){
    ...
    case "GetFeature":
      ...
      print '<?xml version="1.0" encoding="UTF-8"?>';
      echo "\n";
      writeTag("open","gml","featureCollection",$atts,True,True);
      $depth++;
      $sql2 = "SELECT Min(XMin) AS TXMin,Max(XMax) AS TXMax, Min(YMin) AS TYMin,Max(YMax) AS TYMax ";
      $sql="SELECT *";
      $from = " FROM ".$table;
      ...
      if (!(empty($FEATUREID))){
        if(count($featureids)==1){
          $posPeriod = strpos($featureids[0], ".");
          $FeatureID = substr($featureids[0], $posPeriod+1, strlen($featureids[0]));
          $from .= " WHERE FeatureID=".$FeatureID;
        }
      }
      doSelect($table,$sql,$sql2,$from);
      $depth--;
      writeTag("close","gml","featureCollection",null,True,True);
      break;
    default:
      echo $REQUEST;
      break;
